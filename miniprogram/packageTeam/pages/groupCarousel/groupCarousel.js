Page({
  data: {
    ...
    cardConfig: {
      ...
      minScale: 0.25,
      maxTranslateY: -200,
      minTranslateY: -900,  // 增大间距，让后面卡片更高
      visibleCount: 7,
      bottomOffset: 220 // 底部安全距离
    }
  },
  ...
  calculateCardStyle(index, offset) {
    ...
    // 计算缩放和位移
    // 最新的卡片(index = currentIndex)最大，位于最下方
    // 越往后的卡片越小、越高
    ...
    // 计算显示比例 - 后面只显示顶部1/3，但更平缓
    const showRatio = relativeIndex === 0 ? 1 : Math.max(0.3, (1 - relativeIndex * 0.1));
  ...
  handleSwipe(isSwipeUp) {
    const { teams, currentIndex } = this.data;
    ...
    if (isSwipeUp) {
      ...
      // 向下滑，显示上一个
      newIndex = (currentIndex - 1 + teams.length) % teams.length;
    }
